...
p2SString j1App::LoadMap(pugi::xml_node& config)const {
	...
	for (pugi::xml_node file = config.child("map").child("file"); file; file = file.next_sibling("file")) {

		if (file.attribute("loaded").as_bool() == true)
			name = file.attribute("name").as_string();

	}

	if (name == nullptr)
		LOG("Could not load TMX map, because there's no value in true in the XML map node.");
